...
//-------------------------------------------------------
    Tire* ptr2Tire = new Tire; //create new pointer
    if (ptr2Tire->isDroped() && newLocation == "meadow") // set condition when tire is droped and new location is meadow
    {
        ptr2Tire->drop = false;                          //set static function to false
        location->get_contents().push("tire");           //push tire to listOfthing
    }
//-------------------------------------------------------
...
//-------------------------------------
            Tire* ptr2Tire = new Tire;
            ptr2Tire->dropIt();//tire droped cout A tire droped
            delete ptr2Tire;
//-------------------------------------
        }
        return true;
